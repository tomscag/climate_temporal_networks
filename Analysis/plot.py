...
if __name__ == "__main__":
    ...
    finput = folderinput + filelist[6]
    baseline = np.arange(1970,1990)  # 2022,2042    1970,1990
    # baseline = np.arange(1970,1990)
    
    variat_percnt = False
    lw_connectivity = True   # If True is Lorenzo's method
    ...
    #                                         finput, years, baseline, variat_percnt, lw_connectivity)
    ...
    # string += "width_connect_" if lw_connectivity else "width_variat_"
    ...
    # Save figure
    string = "variat_percnt_30yrs" if variat_percnt else "variat_"
    string += "width_connect_" if lw_connectivity else "width_variat_"
    string += finput.split("Output/")[1]
    plt.savefig(f"{resfolder}{string}.png",
                dpi=300,
